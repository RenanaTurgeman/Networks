...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        proxy_socket.bind((proxy_host, proxy_port))  # connect the soct to the port
        proxy_socket.listen(4)  # allocate 4 simultaneously request
        # * Fill in start (1)
        # * Fill in end (1)
        ...
        while True:
            try:
                ...
                # the proxy wait for request from client and create socket connection to the client
                # * Fill in end (2)
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            try:
                ...
                client_socket.send(response)  # send response to the client
                client_socket.close()  # close the socket for specify client
                break
                # * Fill in start (4)
                # * Fill in end (4)
